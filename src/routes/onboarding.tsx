...
function Step1({ onNext }: { onNext: () => void }) {
	...
	return (
		<>
			...
			<Footer primary={t("onboarding.start")} onNext={onNext} />
		</>
	);
}
...
function Step2({ onNext }: { onNext: () => void }) {
	...
	const options = [
		{
			icon: "◉",
			title: t("onboarding.location.gps"),
			sub: t("onboarding.location.gps.sub"),
			badge: t("onboarding.location.gps.badge"),
		},
		{
			icon: "◎",
			title: t("onboarding.location.manual"),
			sub: t("onboarding.location.manual.sub"),
			badge: null,
		},
	];
	...
	return (
		<>
			...
			<Footer
				primary={t("onboarding.location.allow")}
				secondary={t("onboarding.location.manual")}
				onNext={onNext}
				accent="#2f8a6b"
			/>
		</>
...
function Step3({ onNext }: { onNext: () => void }) {
	...
	return (
		<>
					...
					<div className="flex items-center justify-between">
						<div>
							<div className="text-[13px] font-semibold text-ink">
								◉ {t("onboarding.notify.neighbor")}
							</div>
							<div className="text-[11px] text-mint mt-0.5">
								{t("onboarding.notify.neighbor.sub")}
							</div>
						</div>
			...
			<Footer
				primary={t("onboarding.interests.next", { count: selected.length })}
				onNext={onNext}
			/>
		</>
	);
...
function Step4({ onNext }: { onNext: () => void }) {
	...
	const items = [
		{
			icon: "◉",
			title: t("onboarding.notify.neighbor"),
			sub: t("onboarding.notify.neighbor.sub"),
			bg: "#e3f0ea",
			color: "#2f8a6b",
		},
		{
			icon: "♪",
			title: t("onboarding.notify.interests"),
			sub: t("onboarding.notify.interests.sub"),
			bg: "#eae7fb",
			color: "#4d3aef",
		},
		{
			icon: "✦",
			title: t("onboarding.notify.invite"),
			sub: t("onboarding.notify.invite.sub"),
			bg: "#fde8e4",
			color: "#ff6d5a",
		},
	];
	...
	return (
		<>
			<div className="flex-1 px-[22px] pt-4 overflow-y-auto scrollbar-hide">
				...
				<h2 className="font-serif text-[28px] font-medium tracking-[-0.025em] leading-[1.05] mb-[10px]">
					{t("onboarding.notify.h")}
				</h2>
				<p className="text-[13px] text-dim leading-[1.5] mb-5">
					{t("onboarding.notify.desc")}
				</p>
			...
			<Footer
				primary={t("onboarding.notify.enable")}
				secondary={t("onboarding.notify.skip")}
				onNext={onNext}
			/>
		</>
	);
			...
			<div className="flex-1 px-[22px] pt-4 overflow-y-auto scrollbar-hide">
				...
				<div className="px-4 py-[14px] rounded-xl bg-vis-nbr border border-mint/20">
					...
					<div className="text-[13px] text-ink leading-[1.5]">
						{t("onboarding.done.tip.text")}
					</div>
				</div>
			</div>
			<Footer primary={t("onboarding.done.open")} onNext={onDone} />
		</>
	);
}
