...
import { byId, catLabel, formatDate, formatPrice } from "#/data/events";
...
function EventDetail() {
	...
	const accentClass = isPrivate ? "text-coral" : "text-violet";
	const ctaBg = isPrivate
		? "bg-coral shadow-[0_8px_22px_rgba(255,109,90,0.35)]"
		: "bg-violet shadow-[0_8px_22px_rgba(77,58,239,0.35)]";

	const ctaLabel = isPrivate ? t("event.cta.private") : t("event.cta.public");

	const META = [
		{
			icon: "◷",
			label: t("event.meta.datetime"),
			value: `${formatDate(event.date, lang)} · ${event.time}`,
		},
		{ icon: "◎", label: t("event.meta.venue"), value: event.venue[lang] },
		{ icon: "◉", label: t("event.meta.district"), value: event.district[lang] },
		{
			icon: "◈",
			label: t("event.meta.ticket"),
			value: formatPrice(event.priceFrom, event.currency, lang),
		},
	];

	const PRIVATE_RULES = [
		t("event.private.rule.1"),
		t("event.private.rule.2"),
		t("event.private.rule.3"),
	];
			...
			{isDesktop ? (
				...
				<div className="flex h-full overflow-hidden">
					...
					<div className="flex-1 overflow-y-auto scrollbar-hide">
						{/* Cover */}
						<div
							className="relative h-[320px] shrink-0"
							style={{ background: event.img }}
						>
							<div
								...
								<div className="absolute bottom-[18px] left-4 right-4 bg-[rgba(250,248,244,0.95)] rounded-[10px] px-[14px] py-[10px] flex items-center gap-[10px] border border-[rgba(255,109,90,0.3)]">
									...
									<div>
										<div className="text-[11px] font-bold text-coral">
											{t("event.private.invite")}
										</div>
										<div className="text-[11px] text-dim">
											{t("event.private.invited", {
							...
							{!isPrivate && (
								<div className="absolute bottom-6 left-8 right-8">
									<div
										className={cn(
											"text-[10px] tracking-[0.14em] uppercase font-bold mb-1.5 text-white opacity-80",
										)}
									>
						...
						<div className="px-8 py-6 pb-12">
							{isPrivate && (
								<>
									<div
										className={cn(
											"text-[10px] tracking-[0.14em] uppercase font-bold mb-2",
											accentClass,
										)}
									>
										{catLabel(event.cat, lang)}
							...
							<div className="flex items-center gap-2 mb-5">
								...
								{event.rating > 0 && (
									<div className="ml-auto flex items-center gap-2 text-[12px] text-dim">
										...
										<span>·</span>
										<span>
											{event.attendees.toLocaleString()} {t("event.going")}
										</span>
										{event.age > 0 && (
											<>
												<span>·</span>
							...
							<div className="grid grid-cols-4 gap-3 mb-5">
								{META.map((item) => (
									<div
										key={item.label}
										className="bg-white rounded-[10px] px-3 py-[10px] border border-rule"
									>
							...
							{/* Capacity bar */}
							{event.capacity && (
								<div className="bg-vis-pub rounded-[10px] px-[14px] py-3 mb-5 border border-[rgba(77,58,239,0.15)]">
									<div className="flex justify-between items-center mb-2">
										<div className="text-[12px] font-semibold text-violet">
											{event.attendees} / {event.capacity} {t("event.capacity")}
										</div>
										<div className="text-[11px] text-violet">
											{event.capacity - event.attendees}{" "}
											{t("event.capacity.left")}
										</div>
									</div>
									<div className="h-1 bg-[rgba(77,58,239,0.15)] rounded-full overflow-hidden">
										<div
							...
							{/* Description */}
							<p className="text-[15px] leading-[1.65] text-soft-ink mb-5 m-0">
								{event.blurb[lang]}
							</p>
							...
							{isPrivate && (
								<div className="bg-vis-prv rounded-xl px-4 py-[14px] mb-5 border border-[rgba(255,109,90,0.2)]">
									<div className="text-[11px] font-bold tracking-[0.1em] uppercase text-coral mb-3">
										✦ {t("event.private.rules")}
									</div>
									{PRIVATE_RULES.map((rule) => (
										<div
											key={rule}
											className="flex gap-2 text-[12px] text-coral mb-[6px]"
										>
											<span>·</span>
											<span>{rule}</span>
										</div>
					...
					<div className="w-[300px] shrink-0 border-l border-rule bg-paper2 flex flex-col px-5 py-6 overflow-y-auto">
						...
						<div className="flex flex-col gap-[10px] mb-5">
							{META.map((item) => (
								...
								>
									<span className="text-dim w-4 shrink-0 mt-px">
										{item.icon}
									</span>
									<div>
										<div className="text-[10px] text-dim leading-none mb-0.5">
											{item.label}
										</div>
						...
						{/* Capacity */}
						{event.capacity && (
							<div className="bg-vis-pub rounded-[10px] px-3 py-[10px] mb-4 border border-[rgba(77,58,239,0.15)]">
								<div className="flex justify-between mb-2 text-[11px] text-violet font-semibold">
									<span>
						...
						<div className="mt-auto flex flex-col gap-2">
							<button
								type="button"
								className={cn(
									"w-full px-5 py-[14px] rounded-xl text-white font-bold text-[14px] border-none cursor-pointer flex items-center justify-between",
									ctaBg,
								)}
							>
								<span>{ctaLabel}</span>
							...
							<button
								type="button"
								className="w-full py-[10px] text-center text-xs text-dim font-medium bg-transparent border-none cursor-pointer"
							>
								{t("event.save")} ♡
							</button>
						</div>
				...
				<div className="flex-1 flex flex-col overflow-hidden">
					<div className="flex-1 overflow-y-auto scrollbar-hide">
						{/* Cover */}
						<div
							className="relative h-[260px] shrink-0"
							style={{ background: event.img }}
						>
							<div
								className="absolute inset-0"
						...
						<div className="px-[18px] pt-5 pb-6">
							...
							{/* Capacity */}
							{event.capacity && (
								<div className="bg-vis-pub rounded-[10px] px-[14px] py-3 mb-4 border border-[rgba(77,58,239,0.15)]">
									<div className="flex justify-between items-center mb-2">
										<div className="text-[12px] font-semibold text-violet">
											{event.attendees} / {event.capacity} {t("event.capacity")}
										</div>
							...
							{/* Description */}
							<p className="text-[14px] leading-[1.65] text-soft-ink mb-4 m-0">
								{event.blurb[lang]}
							</p>
							...
							{isPrivate && (
								<div className="bg-vis-prv rounded-xl px-4 py-[14px] mb-4 border border-[rgba(255,109,90,0.2)]">
									...
									{PRIVATE_RULES.map((rule) => (
										<div
											key={rule}
											className="flex gap-2 text-xs text-coral mb-[6px]"
										>
											<span>·</span>
											<span>{rule}</span>
										</div>
					...
					<div className="px-[14px] pt-3 pb-4 bg-paper border-t border-rule shrink-0">
						<button
							type="button"
							className={cn(
								"w-full px-5 py-[15px] rounded-[14px] text-white text-[14px] font-bold tracking-[0.02em] border-none cursor-pointer flex items-center justify-between",
								ctaBg,
							)}
						>
							<span>{ctaLabel}</span>
